 ...
 *     description: 내 커피 컬렉션 저장 관련 API
 */
 ...
 *               p_analysis_id: { type: integer, minimum: 1 }
 ...
 *               type: object
 *               properties:
 *                 p_result_code: { type: string, enum: ["SUCCESS","ERROR","INVALID_PARAMETER","MISSING_NAME","MISSING_COMMENT","DUPLICATE_NAME","NO_PERMISSION","ANALYSIS_NOT_FOUND"] }
 *                 p_result_message: { type: string }
 *                 p_collection_id: { type: integer, nullable: true }
 */

// 마운트: app.use("/collections", router) 기준 → 실제 경로는 POST /collections/save
router.post("/save", async (req, res) => {
  ...
  const p_analysis_id = toInt(req.body.p_analysis_id);
  ...
  if (!Number.isInteger(p_user_id) || p_user_id <= 0 || !Number.isInteger(p_analysis_id) || p_analysis_id <= 0) {
    return res.status(200).json({ p_result_code: "INVALID_PARAMETER", p_result_message: "user_id and analysis_id must be positive integers" });
  }
  // ──────────────────────────────────────────────────────────────
  ...
  try {
    ...
    // 출력 변수
    request.output("p_collection_id", sql.Int);
    ...
    return res.status(200).json({
      ...
      p_collection_id: result.output.p_collection_id,
    });
  } catch (err) {
    // [ADD] Duplicate mapping (MSSQL 2601/2627 or UNIQUE KEY)
    const msg = (err && err.message) ? err.message : "";
    const isDup =
      (err && (err.number === 2601 || err.number === 2627)) ||
      /Violation of UNIQUE KEY constraint/i.test(msg);
    if (isDup) {
      return res.status(200).json({
        p_result_code: "DUPLICATE_NAME",
        p_result_message: "Duplicate collection name for this user",
      });
    }

    return res.status(200).json({
      p_result_code: "ERROR",
      p_result_message: msg,
    });
  }
});
